...
#################################
roi = (128, 128, 64)  # Puedes ajustarlo a (128, 128, 128) si la VRAM lo permite
batch_size = 1
sw_batch_size = 2
infer_overlap = 0.5
max_epochs = 100
...
config_train = SimpleNamespace(
    ...
    pipeline="Pipeline 1: Tumor Core + Edema Total",
    network="SwinUNETR",
    use_v2=use_v2,
)
...
train_transform = transforms.Compose([
    ...
    # Formateo de Etiquetas a 2 Canales (TC, Edema)
    ConvertToMultiChannelPipeline1d(keys=["label"]),
    
    # --- Aumentación Espacial CORREGIDA ---
    transforms.CropForegroundd(keys=["image", "label"], source_key="image"),
    
    # NUEVO: Paddeamos con ceros si el cerebro recortado quedó más pequeño que el ROI
...
val_transform = transforms.Compose([
    ...
    # 1. Unificación de canales
    ImputeMissingChannelsd(keys=["image"]),
    ConvertToMultiChannelPipeline1d(keys=["label"]),
    
    # 2. Recorte inteligente: Quita el aire negro, deja TODO el volumen del cerebro
    transforms.CropForegroundd(keys=["image", "label"], source_key="image"),
    
    # 3. Colchón de seguridad: Si un cerebro es inusualmente pequeño, lo rellena 
    # simétricamente con ceros para que no rompa la ventana de inferencia
    transforms.SpatialPadd(keys=["image", "label"], spatial_size=roi),
...
# OPCIONAL: Cargar pesos previos si deseas usarlos como punto de partida
...
def train_epoch(model, loader, optimizer, epoch, loss_func):
    ...
    for idx, batch_data in enumerate(loader):
        ...
        loss.backward()
        optimizer.step()
        optimizer.zero_grad() # Importante: resetear gradientes
...
def main(config_train):
    # Ajustar estas rutas según la ubicación final en tu servidor/entorno
